...
<html>
	<head>		
	    ...
	    <link rel="stylesheet" href="css/forgot-password1.css">
...
<body>
            ...
            <form method="post" class="login active">
				<div class="right-box">
					...
					<div class="right-group" style="border: none !important;">
						<input type="email" id="uname" name="email" value="<?php echo $_SESSION['admin_email']; ?>" disabled style="border-bottom: 2px solid rgba(53, 184, 118, 0.6);" >
                		</div>
    ...
    if (isset($_POST["resetpass"]))
    {
		...
		if($pass == $cpass){
			$s= " SELECT * from admin_tbl where admin_email= '$email' " ;
			$res = mysqli_query($conn, $s);
			$r= mysqli_fetch_array($res);
			if($verification_code==$r['admin_verif_code']){
				if(password_verify($pass, $r['client_pass'])){
        			...
        			$sql = "UPDATE admin_tbl  SET admin_pass = '$encrypted_password'   WHERE admin_email = '" . $email . "' AND admin_verif_code = '" . $verification_code . "'";
